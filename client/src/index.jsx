...
class App extends React.Component {
  ...
  getEstimatedDepartureTimes() {
    axios.get('http://api.bart.gov/api/etd.aspx', {
      ...
      const etd = {};
      const { time } = res.data.root;
      res.data.root.station[0].etd.forEach(route => etd[route.destination] = route.estimate.map(train => train.minutes));
      this.setState({ etd, time })
      ...
      <div>
        {this.state.isLoggedIn ? (
          <div>
            ...
            <StationSchedule etd={this.state.etd} />
          </div>
